...
// Points taken from http://www.dtcenter.org/met/users/docs/write_ups/circle_fit.pdf
	...
	func testShouldCalculateCentroid() {
		...
		XCTAssertEqualWithAccuracy(centroid.x, CGFloat(1.50), accuracy: 0.01, "Centroid not calculated correctly")
		XCTAssertEqualWithAccuracy(centroid.y, CGFloat(3.25), accuracy: 0.01, "Centroid not calculated correctly")
	}
	...
	func testShouldCalculateSums() {
		...
		XCTAssertEqualWithAccuracy(cf.calcSumUU(uv),		CGFloat(7.00), accuracy: 0.01, "sUU not calculated correctly")
		XCTAssertEqualWithAccuracy(cf.calcSumUV(uv),		CGFloat(21.00), accuracy: 0.01, "sUV not calculated correctly")
		XCTAssertEqualWithAccuracy(cf.calcSumVV(uv),		CGFloat(68.25), accuracy: 0.01, "sVV not calculated correctly")
		XCTAssertEqualWithAccuracy(cf.calcSumUUU(uv),	CGFloat(0.00), accuracy: 0.01, "sUUU not calculated correctly")
		XCTAssertEqualWithAccuracy(cf.calcSumVVV(uv),	CGFloat(143.81), accuracy: 0.01, "sVVV not calculated correctly")
		XCTAssertEqualWithAccuracy(cf.calcSumUVV(uv),	CGFloat(31.50), accuracy: 0.01, "sUVV not calculated correctly")
		XCTAssertEqualWithAccuracy(cf.calcSumUUV(uv),	CGFloat(5.25), accuracy: 0.01, "sVUU not calculated correctly")
	}
